...
export const employeeFindById = async (id: number): Promise<IEmployee | null> => {
  const res = await pool.query(`select e.*,d.name as department_name,(COALESCE(e2.first_name, '') || ' ' || COALESCE(e2.last_name, '')) as manager_name from employee e join department d on d.id=e.department_id left join employee e2 on e2.id=e.manager_id where e.id = $1`, [id]);
